...
#define DEFAULT_SERIALIZE8(a,b,c,d,e,f,g,h) std::size_t to_bytes(char* ret) const { \
        ...
        return sa + sb + sc + sd + se + sf + sg + mutils::to_bytes(h,ret + sa + sb + sc + sd + se + sf + sg); \
    }                                                                   \
    std::size_t bytes_size() const {                                            \
...
#define DEFAULT_DESERIALIZE9(Name,a,b,c,d,e,f,g,h)                            \
    static std::unique_ptr<Name> from_bytes(mutils::DeserializationManager* p, char const * v){                 \
        ...
        return std::make_unique<Name>(*a2,*b2,*c2,*d2,*e2,*f2,*g2,*(mutils::from_bytes<std::decay_t<decltype(h)> >(p,v + size_a2 + size_b2 + size_c2 + size_d2 + size_e2 + size_f2 + mutils::bytes_size(*g2)))); \
    }
